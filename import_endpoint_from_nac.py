...
if __name__ == "__main__":
    ...
    if (len(sys.argv) != 2):
        sys.exit('\nusage: ./script.py <source-nac-list-exported-records>\n')
    ...
    for line_entry in iter(f):
        ...
        if '#' in line_entry:
            ...
            results = None
            try:
                utils_endpoint.create_endpoint(creds_edit,creds_read,mac_address,role,desc)
                #thread_call = threading.Thread(
                #    target=utils_endpoint.create_endpoint,
