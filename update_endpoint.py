...
if __name__ == "__main__":
    ...
    try:
        ...
        #creds = accounts_db.accounts_pan[0]
    except Exception, e:
        print e
        sys.exit()
    ...
    for line_entry in iter(f):
        ...
        if '#' in line_entry:
            ...
            results = None
            try:
                results = utils_endpoint.update_endpoint(creds_edit,creds_read,mac_address,role,desc)
                if results:
                    print utils_endpoint.json_pretty_print(results)
                #thread_call = threading.Thread(
                #    target=utils_endpoint.update_endpoint,
                #    args=(creds_edit,creds_read, mac_address,role,desc))
